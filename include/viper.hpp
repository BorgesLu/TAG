...
#define STORE_ORDER std::memory_order_release
    ...
    namespace internal
    {
        ...
        { //定长数据，一个page内的 slots
            ...
            {
                current_page_size += PAGE_SIZE; //对于大KV分配更大的Page
            }
        ...
        struct VarEntryAccessor
        {
            ...
            {
                const VarSizeEntry *entry = reinterpret_cast<const VarSizeEntry *>(raw_entry); //读头部，
                is_set = entry->is_set;
        ...
        { // string 类型的page
            ...
            std::array<char, DATA_SIZE> data; // char 数组来保存数据
        ...
        { // block的结构
             ...
             */
            std::array<VPage, num_pages> v_pages; //使用一个array 保存每一个page
    ...
    struct ViperBase
    {
        ...
        std::vector<ViperFileMapping> v_mappings; //每一个block的起始地址？
    };
    ...
    class Viper
    {
        ...
        static_assert(BLOCK_SIZE % v_page_size == 0, "Page needs to fit into block."); // satic_assert 为 false,编译时就会报错
    ...
    public:
        ...
        public:
            ...
            //属性图场景。 key的类型是 unint_64t,
            // value 是 string
            bool put(const uint64_t &IndexKey, const std::string &value);

            bool get(const uint64_t &key, V *value);

            //   bool put(const std::string& key, const std::string& value, const int32_t& schemaID);

            void scan_test();
            void comment_scan_test();
            void scan_from_index_test(uint64_t &startKey, uint64_t &endKey);
            ...
            //scan op
            //return tht key(startkey,endkey)
            bool scan_for_key(uint64_t &srcKey,uint64_t &endKey,std::vector<uint64_t> &result);
        ...
        protected:
            ...
            //使用vector 来保存page_, 用于scan
            std::vector<VPage *> v_pages_sacn;
    ...
    protected:
        ...
        moodycamel::ConcurrentQueue<block_size_t> free_blocks_; //使用无锁的queue，维护blocks的信息
    ...
    template <typename K, typename V>
    Viper<K, V>::Viper(ViperBase v_base, const std::filesystem::path pool_dir, const bool owns_pool, const ViperConfig v_config) : // v_base_{v_base}, map_{131072}, owns_pool_{owns_pool}, v_config_{v_config}, pool_dir_{pool_dir},
                                                                                                                                   v_base_{v_base}, map_{512, 512}, owns_pool_{owns_pool}, v_config_{v_config}, pool_dir_{pool_dir},
                                                                                                                                   ...
                                                                                                                                   num_recovery_threads_{v_config.num_recovery_threads}
    {
        ...
        //   std::cout<<"file_descriptor :\t"<<v_base_.file_descriptor<<std::endl;  // -1
        //   std::cout<<"file based:\t"<<v_base_.is_file_based<<std::endl;
        //   std::cout<<"map size:\t"<<v_base_.v_mappings.size()<<std::endl;
                                 ...
                                 bool is_new_pool, ViperConfig v_config, const size_t block_size)
    {
        ...
        for (size_t chunk_num = 0; chunk_num < num_alloc_chunks; ++chunk_num)
        {
            ...
            void *pmem_addr = mmap(nullptr, alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED, data_fd, 0);
            MMAP_CHECK(pmem_addr)
        ...
        if (is_new_pool)
        {
            void *metadata_addr = mmap(nullptr, alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED, meta_fd, 0);
            MMAP_CHECK(metadata_addr)
    ...
    void Viper<K, V>::get_block_based_access(Client *client)
    {
        ...
        // std::cout<<"free block size\t"<<free_blocks_.size_approx()<<std::endl;
        if (!free_blocks_.try_dequeue(client_block))
        { // queue 为 empty， get new block
        ...
        //   std::cout<<"block based, line 919\t\n";
        //  std::cout<<"client_block\t"<<client_block<<"\n";
    ...
    // page based scan test
    // BI-1-Post
    template <>
    void Viper<std::string, std::string>::Client::scan_test()
    {
        const size_t meta_size = sizeof(internal::VarSizeEntry::size_info); //每条记录的元信息
        //int male{0};
        //int female{0};
        int group_1{0};
        int group_2{0};
        ...
        int group_4{0};
        int postNum{0};
        for (auto iter = v_pages_sacn.begin(); iter != v_pages_sacn.end(); iter++)
        {
            // std::cout << "v_pages_sacn size: " << v_pages_sacn.size() << std::endl;
            VPage *v_page = (*iter);
            v_page->lock();
            const char *raw_data = v_page->data.data(); //返回指向数组对象的第一个元素的指针
            ...
            { //遍历当前的page
                ...
                data_offset_size_t data_skip = meta_size + var_entry.key_size + var_entry.value_size; //当前记录与下一条记录的距离
                ...
                if (var_entry.is_set)
                {
                    ...
                    uint16_t start{0};
                    uint16_t low = 0x000000ff; //直接移位会出错（当最高位为1时）
                    for(int i =0 ;i < 7;++i){
                       uint16_t le = value.at(i);
                       le &= low;
                       start += le;
                   }
                    //size_t length = value.at(6);
                     uint16_t length = value.at(7);
                     length &= low;
                   
            
                    std::string connnetLength = value.substr(start, length);
                    //std::cout << "connet is: " <<connnetLength<< "\n";
                   // std::cout << "lineNum: " <<postNum<< "\n";
                   // std::cout << "value: " <<value<< "\n";
                   // std::cout << "start: " <<start<< "\n";
                    int levle{0};
                    try{ levle = stoi(connnetLength);}
                    catch(...){
                     levle = 0;
                    }
                  // int levle = 40;
     ...
     std::cout << "g1-g2-g3=g4:" <<group_1<<"-"<<group_2<<"-"<<group_3<<"-"<<group_4<< std::endl;
     std::cout << "postNum" <<postNum<< std::endl;
    }
...
// page based scan test
    // BI-1-Post
    template <>
    void Viper<std::string, std::string>::Client::comment_scan_test()
    {
        const size_t meta_size = sizeof(internal::VarSizeEntry::size_info); //每条记录的元信息
        ...
        for (auto iter = v_pages_sacn.begin(); iter != v_pages_sacn.end(); iter++)
        {
            ...
            { //遍历当前的page
                ...
                if (var_entry.is_set)
                {
                    ...
                    //size_t length = value.at(6);
                     uint16_t length = value.at(5);
                     length &= low;

                    // std::cout << "gender is: " << value.substr(start, length) << "\n";
                    std::string connnetLength = value.substr(start, length);
                   // int levle = stoi(connnetLength);
                     int levle{0};
                    try{ levle = stoi(connnetLength);}
                    catch(...){
                     levle = 0;
         ...
         std::cout << "comm: g1-g2-g3-g4: " <<group_1<<"-"<<group_2<<"-"<<group_3<<"-"<<group_4<< std::endl;
     std::cout << "postNum" <<commentNum<< std::endl;
    }


   template <typename K, typename V>
    void Viper<K, V>::Client::scan_from_index_test(uint64_t &startKey, uint64_t &endKey)
    {
        vector<KVOffset> addrList;
        //返会数据的的指针
        this->viper_.map_.scan(startKey, endKey, addrList);
        int male{0};
        int female{0};
        std::string value;
        for (auto n : addrList)
        ...
        std::cout << "male is:" << male << "\n";
        std::cout << "female is:" << female << "\n";
    }
    template <>
    bool Viper<std::string, std::string>::Client::put(const std::string &key, const std::string &value, const bool delete_old)
    {
        ...
        int64_t map_key = std::stol(key); // map 也就是BP-Tree只支持 int64 类型的 key
        // old_offset =
        this->viper_.map_.Insert(map_key, var_offset);
    ...
    template <>
    bool Viper<std::string, std::string>::Client::put(const uint64_t &IndexKey, const std::string &value)
    {
        v_page_->lock();
        VPage *start_v_page = v_page_;

        //保存page的地址 需要避免重复 push
        auto iter = find(v_pages_sacn.begin(), v_pages_sacn.end(), v_page_);
        if (iter == v_pages_sacn.end())
        {
            v_pages_sacn.push_back(v_page_); //没有找到，加入队列
        }

        // uint64_t map_key = key; //
        std::string key = std::to_string(IndexKey);
        ...
        // int64_t map_key = std::stol(key); //map 也就是BP-Tree只支持 int64 类型的 key
        // old_offset =
        //std::cout << "Insert key:" << IndexKey << "\n";
        this->viper_.map_.Insert(IndexKey, var_offset);
        // is_new_item = old_offset.is_tombstone();
        size_delta_++;
    ...
    template <typename K, typename V>
    bool Viper<K, V>::Client::get(const uint64_t &key, V *value)
    {
        ...
        while (true)
        {
            // std::cout<<"L1357 map_key is:\t"<<map_key<<std::endl;
            KVOffset kv_offset = this->viper_.map_.Get(key);
    ...
    bool Viper<K, V>::Client::get(const K &key, V *value)
    {
        ...
        while (true)
        {
            ...
            // KVOffset kv_offset = this->viper_.map_.Get(key);
            // uint64_t map_key = std::stol(key);
            // std::cout<<"L1231 map_key is:\t"<<map_key<<std::endl;
            KVOffset kv_offset = this->viper_.map_.Get(key);
            if (kv_offset.is_tombstone())
            {
                return false;
    ...
    bool Viper<K, V>::ReadOnlyClient::get(const K &key, V *value) const
    {
        ...
        while (true)
        {
            // KVOffset kv_offset = this->viper_.map_.Get(key, key_check_fn);
            uint64_t map_key = std::stol(key);
            KVOffset kv_offset = this->viper_.map_.Get(map_key);
            if (kv_offset.is_tombstone())
            {
                return false;
            }
     ...
     */
    template <typename K, typename V>
    bool Viper<K, V>::Client::scan_for_key(uint64_t &startKey,uint64_t &endKey,std::vector<uint64_t> &result)
    {
    
        return this->viper_.map_.scan_key(startKey,endKey,result);
    }
    ...
    bool Viper<K, V>::Client::get_khop_neighbors_from_index(uint64_t &srcVID, int &hop, int &count)
    {
        ...
        /* std::vector<uint64_t> adjListVID;
         this->viper_.map_.getNeighborsFromIndex(srcVID,adjListVID);
         count += adjListVID.size();
         //std::cout<<"count in viper is: "<<count<<"\n";
         for(auto ID :adjListVID){
             std::vector<uint64_t> tempAdjListVID;
             this->viper_.map_.getNeighborsFromIndex(ID,tempAdjListVID);
            // std::cout<<"ID is: "<<ID<<" neighbors size is: "<<tempAdjListVID.size()<<"\n";
             count += tempAdjListVID.size();
    ...
    bool Viper<K, V>::Client::get_khop_neighbors_from_data(uint64_t &srcVID, int &count)
    {
        ...
        /*
          count +=adjListAddress.size();
           //std::cout<<"count in data "<<count<<std::endl;
          uint64_t value;
           std::vector<uint64_t> desVID;
          for(auto n : adjListAddress){
             //   count++;
                get_value_from_offset(n, &value);

                desVID.push_back(value);
          }
           uint64_t tempValue;
          for(auto var : desVID){
              vector<KVOffset> tempadjListAddress;
               this->viper_.map_.getNeighbors(var,tempadjListAddress);
              // std::cout<<"var is "<<var<<" neighbor count is "<<tempadjListAddress.size()<<"\n";
               count += tempadjListAddress.size();
               for(auto n:tempadjListAddress){
                get_value_from_offset(n, &tempValue);
               }
          }
      */
    }
    ...
    bool Viper<K, V>::Client::update(const K &key, UpdateFn update_fn)
    {
        ...
        while (true)
        {
            ...
            const KVOffset kv_offset = this->viper_.map_.Get(key);
            if (kv_offset.is_tombstone())
            {
                return false;
            }
    ...
    void Viper<K, V>::Client::free_occupied_slot(const KVOffset offset_to_delete, const K &key, const bool delete_offset)
    {
        ...
        if (v_block_number_ == block_number && v_page_number_ == page_number)
        {
            ...
            if (delete_offset)
            {
                ...
                uint64_t map_key = std::stol(key);
                this->viper_.map_.Insert(map_key, IndexV::NONE());
            }
            --size_delta_;
            return;
        }
    ...
    inline bool Viper<std::string, std::string>::Client::get_value_from_offset(const KVOffset offset, std::string *value)
    {
        ...
        const char *raw_data = &v_page.data[data_offset]; //记录的起始位置
        internal::VarEntryAccessor var_entry{raw_data};
        if (var_entry.value_size == 0)
        {
            // Value is on next page
    ...
    void Viper<std::string, std::string>::compact(Client &client, VPageBlock *v_block)
    {
        ...
        while (true)
        {
            ...
            if (var_entry.is_set)
            {
                ...
                uint64_t map_key = std::stol(key);
                IndexV offset = map_.Get(map_key);
